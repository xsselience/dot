...
public class Explodable : MonoBehaviour
{
    ...
    public void ExplodeAtPoint(Vector2 hitPoint)
    {
        explode(); // 生成碎片
        ...
        foreach (GameObject frag in fragments)
        {
            ...
            if (rb != null)
            {
                Vector2 dir = (frag.transform.position - (Vector3)hitPoint).normalized;
    ...
    private void generateFragments()
    {
        ...
        {
            var srFrag = p.GetComponent<SpriteRenderer>();
            if (srFrag != null)
            {
                srFrag.color = GetComponent<SpriteRenderer>().color; // 当时的障碍物颜色
            }

            if (p.GetComponent<FragmentAutoDestroy>() == null)
            {
                p.AddComponent<FragmentAutoDestroy>();
            }
        }
